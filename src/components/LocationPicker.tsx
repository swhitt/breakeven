  ...
  return (
    <div ref={boxRef} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-label={`Change metro, currently ${selected.metro}`}
        onClick={() => {
          setOpen((o) => !o);
          setActive(0);
        }}
      ...
      >
        <span className="flex items-center gap-2">
          <svg className="h-4 w-4 text-rent" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path
              fillRule="evenodd"
      ...
      {open && (
        <div className="absolute z-30 mt-1.5 w-full overflow-hidden rounded-xl border border-line bg-surface shadow-xl shadow-black/10">
          ...
          <ul id={listId} role="listbox" aria-label="Metros" className="max-h-72 overflow-y-auto py-1">
            ...
            {results.length === 0 && (
              <li className="px-3 py-3 text-sm text-muted">No metros match.</li>
            )}
          </ul>
        </div>
      )}
